...
namespace AcryGen.Models
{
    public class AcGenerator
    {
        ...
        public string[] getAcronym(string acronym, bool noSwears)
        {
            ...
            for (var i = 0; i < letters.Length; i++)
            {
                ...
                //generate a random number in the length of the file and pull the word at that index
                var ran = new Random().Next(0, lines.Length - 1);
                string line = lines[ran];

                //make first letter of the line uppercase - will help with final formatting and fullstring checks
                string upperWord = char.ToUpper(line[0]) + line.Substring(1);

                //if the user requested no swear words and the picked word is a swear, re-do the current interation
                //   do the same if the picked word is already in the acronym phrase
                if ((noSwears && Array.IndexOf(SWEARS, line) > -1) || fullString.Contains(upperWord))
                {
                    i--;
                    continue;
                }

                fullString = fullString + upperWord + " ";
